...
_Figure 1 - User centred system design principles (Gulliksen et al., 2003)_
...
Quant au site web dans lequel la carte est intégrée, il présente une barre de navigation contenant les sections _About S4D, Projects' Map_ et _FAQ_. De plus, il contient une sorte de cadre au-dessus de la carte qui affiche une multitude de filtres. Les sélections possibles pour ces filtres sont affichées en cliquant sur le menu _dropdown_. En rétrospective, je dois admettre que je n'avais pas encore précisément réfléchi à comment visualiser les filtres appliqués.
...
_Figure 6 - La carte interactive avec les fonctions de filtres appliqués et _hover over_ _
...
La bibliothèque MapLibre GL JS permet la personnalisation des pins. Le design du pin personnalisé a été élaboré de manière itérative à l'aide du LLM, jusqu'à l'obtention d'un résultat satisfaisant.
...
Un point fort de ce projet est que la géovisualisation répond à un réel besoin, qui a été identifié lors de recherches.
...
Egalement à noter au sujet de la base de données, la mise à jour doit se faire manuellement. Pour un nombre de données limité, cela ne prend pas beaucoup de temps et ne pose pour l'instant pas de problème. Toutefois, si la base de données grandit et que les données deviennent plus dynamiques, la mise à jour des données s'avère plus complexe.
